    ...
    return (function e(t, n, r) {
        ...
        1: [function(require, module, exports) {
            ...
            function redo(step=1) {
                ...
                const cp = getLatestCheckpoint(this, redoNo);
                for (currentCp !== cp && (cp.apply(this),
                redo = cp.redo); redo && redo.no <= redoNo; )
                    redo.apply(this),
                    this._undodata.current = redo,
                    redo = redo.next
            }
            function undoTag(name=/.*/, step=1) {
                ...
                let tags, index = (tags = name instanceof RegExp ? this._undodata.tags.filter(tag=>tag.no < current.no && name.test(tag.name)) : this._undodata.tags.filter(tag=>tag.no < current.no && tag.name == name)).length - step;
            ...
            function redoTag(name=/.*/, step=1) {
                ...
                const current = this._undodata.current;
                let tags;
                (tags = name instanceof RegExp ? this._undodata.tags.filter(tag=>tag.no > current.no && name.test(tag.name)) : this._undodata.tags.filter(tag=>tag.no > current.no && tag.name == name)).length <= step - 1 || (this.currentHistoryNo = tags[step - 1].no)
            }
            ...
            function recalcCost(obj) {
                let redo = obj._undodata.checkpoints[obj._undodata.checkpoints.length - 1].redo.next
                  , cost = 0;
                for (; redo; )
                    cost += redo.cost,
                    redo = redo.next;
            ...
            function deleteFutureData(obj) {
                ...
                for (i = tags.length - 1; i >= 0 && !(tags[i].no <= currentNo); i--)
                    ;
                tags.length = i + 1,
                recalcCost(obj)
            }
            ...
            function hook(obj, propertyName, needsCommit) {
                const desc = Object.getOwnPropertyDescriptor(obj.constructor.prototype, propertyName);
                void 0 !== desc && (desc.configurable ? void 0 !== desc.set ? hookAccessor(obj, propertyName, desc) : void 0 !== desc.get || hookFunction(obj, propertyName, needsCommit) : console.error(propertyName + " is not configurable"))
            }
            ...
            function enableUndo(context, options={}) {
                ...
                for (const name of names)
                    -1 === ignoreTriggers.indexOf(name) && hook(context, name, commitTriggers.indexOf(name) >= 0);
            ...
            const resetObject = require("reset-object")
              , ignoreTriggers = ["canvas", "constructor", "createImageData", "createLinearGradient", "createPattern", "createRadialGradient", "getImageData", "getLineDash", "isPointInPath", "isPointInStroke", "measureText", "scrollPathIntoView"]
              , commitTriggers = ["clearRect", "drawFocusIfNeeded", "drawImage", "fill", "fillRect", "fillText", "putImageData", "stroke", "strokeRect", "strokeText"];
            class CheckPoint {
                ...
                putImageData(context) {
                    ...
                    Object.getOwnPropertyDescriptor(context.constructor.prototype, "putImageData").value.bind(context)(this.imageData, 0, 0)
                }
                getContextParameters(context) {
                    const names = Object.getOwnPropertyNames(context.constructor.prototype)
                      , params = {};
                    for (const name of names) {
                        if (-1 !== ignoreTriggers.indexOf(name))
                            continue;
                        const prop = Object.getOwnPropertyDescriptor(context.constructor.prototype, name);
                        prop.get && prop.set && (params[name] = prop.get.bind(context)())
                    }
                ...
                setContextParameters(context) {
                    ...
                    for (const key of keys)
                        Object.getOwnPropertyDescriptor(context.constructor.prototype, key).set.bind(context)(src[key])
                }
                apply(context) {
            ...
            class CommandLog {
                ...
                serialize(funcs) {
                    ...
                    return -1 == index && (funcs.push(this.func),
